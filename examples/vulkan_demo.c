...
static inline float dot_scalar(float ax, float ay, float az, float bx, float by,
                               float bz) {
  return ax * bx + ay * by + az * bz;
}

float intersect_scalar(float ox, float oy, float oz, float dx, float dy,
                       float dz, float cx, float cy, float cz, float r) {
...
void trace_scalar(RayPack *rays, Sphere *spheres, int num_spheres,
  ...
  for (int i = 0; i < NUM_RAYS; i++) {
    ...
    for (int s = 0; s < num_spheres; s++) {
      float t =
          intersect_scalar(rays->ox[i], rays->oy[i], rays->oz[i], rays->dx[i],
                           rays->dy[i], rays->dz[i], spheres[s].cx,
                           spheres[s].cy, spheres[s].cz, spheres[s].r);
      if (t > 0.0f && t < closest)
        closest = t;
    }
...
static inline __m256 dot_simd(__m256 ax, __m256 ay, __m256 az, __m256 bx,
                              __m256 by, __m256 bz) {
...
__m256 intersect_simd(__m256 ox, __m256 oy, __m256 oz, __m256 dx, __m256 dy,
                      __m256 dz, float cx, float cy, float cz, float r) {
  ...
  __m256 b =
      _mm256_mul_ps(_mm256_set1_ps(2.0f), dot_simd(ocx, ocy, ocz, dx, dy, dz));
...
void trace_simd(RayPack *rays, Sphere *spheres, int num_spheres,
  ...
  for (int s = 0; s < num_spheres; s++) {
    for (int i = 0; i < NUM_RAYS; i += 8) {
      ...
      __m256 t = intersect_simd(ox, oy, oz, dx, dy, dz, spheres[s].cx,
                                spheres[s].cy, spheres[s].cz, spheres[s].r);

      __m256 current = _mm256_loadu_ps(&result[i]);
      __m256 closer =
          _mm256_and_ps(_mm256_cmp_ps(t, _mm256_setzero_ps(), _CMP_GT_OQ),
...
void trace_rake(RayPack *rays, Sphere *spheres, int num_spheres,
  ...
  for (int s = 0; s < num_spheres; s++) {
    for (int i = 0; i < NUM_RAYS; i += 8) {
      ...
      __m256 t = intersect_flat(ox, oy, oz, dx, dy, dz, spheres[s].cx,
                                spheres[s].cy, spheres[s].cz, spheres[s].r);
...
static uint32_t find_memory_type(VkPhysicalDevice pdev, uint32_t filter,
...
bool vulkan_init(VulkanState *vk, size_t num_rays) {
  ...
  VkInstanceCreateInfo inst_ci = {.sType =
  ...
  vkGetPhysicalDeviceQueueFamilyProperties(vk->physical_device, &queue_count,
                                           NULL);
  ...
  if (vkCreateDevice(vk->physical_device, &device_ci, NULL, &vk->device) !=
      VK_SUCCESS) {
  ...
  /* Create buffers */
  VkBufferCreateInfo buf_ci = {.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
                               .size = vk->buffer_size,
  ...
  for (int i = 0; i < 6; i++) {
    ...
    total_size +=
        (mem_reqs.size + mem_reqs.alignment - 1) & ~(mem_reqs.alignment - 1);
  }
  ...
  VkMemoryAllocateInfo mem_ai = {
      ...
      .memoryTypeIndex =
          find_memory_type(vk->physical_device, mem_reqs.memoryTypeBits,
  ...
  VkDescriptorSetLayoutCreateInfo layout_ci = {
      ...
      .bindingCount = 8,
      .pBindings = bindings};
  vkCreateDescriptorSetLayout(vk->device, &layout_ci, NULL,
                              &vk->descriptor_layout);

  /* Create descriptor pool and set */
  VkDescriptorPoolSize pool_sizes[] = {{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 7},
  ...
  for (int i = 0; i < 6; i++) {
    buffer_infos[i] = (VkDescriptorBufferInfo){
        .buffer = vk->ray_buffers[i], .offset = 0, .range = vk->buffer_size};
  ...
  buffer_infos[6] = (VkDescriptorBufferInfo){
      .buffer = vk->sphere_buffer, .offset = 0, .range = sizeof(float) * 4};
  ...
  buffer_infos[7] = (VkDescriptorBufferInfo){
      .buffer = vk->result_buffer, .offset = 0, .range = vk->buffer_size};
  ...
  vkCreatePipelineLayout(vk->device, &pipe_layout_ci, NULL,
                         &vk->pipeline_layout);
  ...
  vkCreateComputePipelines(vk->device, VK_NULL_HANDLE, 1, &pipe_ci, NULL,
                           &vk->pipeline);
...
void vulkan_cleanup(VulkanState *vk) {
  ...
  if (vk->descriptor_layout)
    vkDestroyDescriptorSetLayout(vk->device, vk->descriptor_layout, NULL);

  for (int i = 0; i < 6; i++) {
    if (vk->ray_buffers[i])
      vkDestroyBuffer(vk->device, vk->ray_buffers[i], NULL);
  }
  if (vk->sphere_buffer)
    vkDestroyBuffer(vk->device, vk->sphere_buffer, NULL);
  if (vk->result_buffer)
    vkDestroyBuffer(vk->device, vk->result_buffer, NULL);
  if (vk->memory)
...
void vulkan_upload_rays(VulkanState *vk, RayPack *rays) {
  ...
  float *ray_arrays[6] = {rays->ox, rays->oy, rays->oz,
...
void vulkan_dispatch(VulkanState *vk, uint32_t num_rays) {
  ...
  vkCmdBindDescriptorSets(vk->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE,
                          vk->pipeline_layout, 0, 1, &vk->descriptor_set, 0,
                          NULL);
...
void trace_vulkan(VulkanState *vk, RayPack *rays, Sphere *spheres,
                  int num_spheres, float *result) {
...
void render_to_surface(SDL_Surface *surface, float *result, int width,
                       int height) {
...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window =
      SDL_CreateWindow("Rake Raytracer Demo", SDL_WINDOWPOS_CENTERED,
  ...
  /* Setup spheres */
  Sphere spheres[NUM_SPHERES] = {{0.0f, 0.0f, -5.0f, 1.0f},
                                 {-2.0f, 1.0f, -6.0f, 0.8f},
                                 {2.0f, -0.5f, -4.5f, 0.6f},
                                 {-1.0f, -1.0f, -4.0f, 0.5f},
                                 {1.5f, 1.2f, -7.0f, 1.2f}};
  ...
  printf("  C SIMD:      %7.2f ms   (%.1f FPS)  %.2fx speedup\n", simd_time,
         1000.0 / simd_time, scalar_time / simd_time);
  printf("  Rake SIMD:   %7.2f ms   (%.1f FPS)  %.2fx speedup\n", rake_time,
         1000.0 / rake_time, scalar_time / rake_time);
  if (have_vulkan) {
    printf("  Vulkan GPU:  %7.2f ms   (%.1f FPS)  %.2fx speedup\n", vulkan_time,
           1000.0 / vulkan_time, scalar_time / vulkan_time);
  }
  printf("================================================================\n");
  printf("\n");
  ...
  while (running) {
    ...
    switch (mode) {
    case 1:
      trace_scalar(&rays, animated, NUM_SPHERES, result);
      break;
    case 2:
      trace_simd(&rays, animated, NUM_SPHERES, result);
      break;
    case 3:
      trace_rake(&rays, animated, NUM_SPHERES, result);
      break;
    case 4:
      if (have_vulkan)
        trace_vulkan(&vk, &rays, animated, NUM_SPHERES, result);
      break;
    }
